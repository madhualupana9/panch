<x-admin-layout 
    ...
    <div class="max-w-5xl mx-auto py-6">
        <div class="mb-8 flex flex-col sm:flex-row items-center justify-between gap-4">
            <a href="{{ route('admin.drr.index', ['tab' => 'enquiries']) }}" class="inline-flex items-center text-sm font-medium text-gray-600 hover:text-blue-600 transition-all bg-white px-5 py-2.5 rounded-xl shadow-sm border border-gray-200">
                <i class="fas fa-arrow-left mr-2"></i>
                Back to DRR Premium County
            </a>
            
            <form action="{{ route('admin.drr.enquiry.destroy', $enquiry) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this enquiry?');">
                @csrf
                @method('DELETE')
